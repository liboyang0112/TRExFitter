...
void RunSig(const char* inFileName,
    ...
    if (!ws){
        std::string s = wsName;
        WriteErrorStatus("RunSig::RunSig", "Workspace: " + s + " doesn't exist!");
        return;
    }
    ...
    if (!mc){
        ...
        WriteErrorStatus("RunSig::RunSig", "ModelConfig: " + s + " doesn't exist!");
        return;
    }
    RooDataSet* data = (RooDataSet*)ws->data(dataName);
    if (!data){
        std::string s = dataName;
        WriteErrorStatus("RunSig::RunSig", "ModelConfig: " + s + " doesn't exist!");
        return;
    }
    ...
    WriteInfoStatus("RunSig::RunSig", "Setting max function calls");
    ...
    if (doMedian){
        ...
        if (status < 0) {
            WriteInfoStatus("RunSig::RunSig", "Retrying with conditional snapshot at mu=1");
            ws->loadSnapshot("conditionalNuis_0");
            status = minimize(asimov_nll, ws);
            if (status >= 0) WriteInfoStatus("RunSig::RunSig", "Success!");
        }
    ...
    WriteInfoStatus("RunSig::RunSig", "Observed significance: " + std::to_string(obs_sig));
    if (med_sig){
        WriteInfoStatus("RunSig::RunSig", "Median test stat val: " + std::to_string(asimov_q0));
        WriteInfoStatus("RunSig::RunSig", "Median significance:       " + std::to_string(med_sig));
    }
...
int minimize(RooNLLVar* nll, RooWorkspace* combWS){
    ...
    if (status != 0 && status != 1 && strat < 2){
        strat++;
        WriteWarningStatus("RunSig::minimize", "Fit failed with status " + std::to_string(status) + ". Retrying with strategy " + std::to_string(strat));
        minim.setStrategy(strat);
    ...
    if (status != 0 && status != 1){
        WriteWarningStatus("RunSig::minimize", "Fit failed with status " + std::to_string(status));
    }
...
RooDataSet* makeData(RooDataSet* orig, RooSimultaneous* simPdf, const RooArgSet* observables, RooRealVar* firstPOI, double mass, double& mu_min){
    ...
    while ((ds = (RooAbsData*)dataItr->Next())){
        ...
        pdf = simPdf->getPdf(typeName.c_str());
        auto pdf_int = reinterpret_cast<uintptr_t>(pdf);
        WriteDebugStatus("RunSig::makeData", "pdf: " + std::to_string(pdf_int));
        RooArgSet* obs = pdf->getObservables(observables);
        auto obs_int = reinterpret_cast<uintptr_t>(obs);
        WriteDebugStatus("RunSig::makeData", "obs: " + std::to_string(obs_int));
...
//           }
            if (b == 0 && s != 0){
                std::string s = pdf->GetName();
                WriteWarningStatus("RunSig::modelData", "Expecting non-zero signal and zero bg at m=" + std::to_string(firstObs->getVal()) + " in pdf " + s);
            }
            if (s+b <= 0) {
                WriteWarningStatus("RunSig::modelData", "s+b <= 0, continuing");
                continue;
            }
...
void unfoldConstraints(RooArgSet& initial, RooArgSet& final, RooArgSet& obs, RooArgSet& nuis, int& counter){
    if (counter > 50){
        WriteErrorStatus("RunSig::unfoldConstraints", "Couldn't unfold constraints!");
        WriteErrorStatus("RunSig::unfoldConstraints", "Initial:");
        initial.Print("v");
        WriteErrorStatus("RunSig::unfoldConstraints", "");
        WriteErrorStatus("RunSig::unfoldConstraints", "Final: ");
        final.Print("v");
        exit(1);
    }
...
RooDataSet* makeAsimovData(ModelConfig* mc, bool doConditional, RooWorkspace* w, RooNLLVar* conditioning_nll, double mu_val, string* mu_str, string* mu_prof_str, double mu_val_profile, bool doFit){
    ...
    WriteInfoStatus("RunSig::makeAsimovData", "Creating asimov data at mu = " + std::to_string(mu_val) + ", profiling at mu = " + std::to_string(mu_val_profile));
...
//       components->Print();
        ...
        if (components->getSize() > 1){
            WriteErrorStatus("RunSig::makeAsimovData", "Couldn't isolate proper nuisance parameter");
            return NULL;
        }
        ...
        if (!thisNui || !thisGlob){
            std::string s = pdf->GetName();
            WriteWarningStatus("RunSig::makeAsimovData", "Couldn't find nui or glob for constraint: " + s);
            //return;
            continue;
        }

        std::string s = thisNui->GetName();
        std::string s2 = thisGlob->GetName();
        std::string s3 = pdf->GetName();
        WriteDebugStatus("RunSig::makeAsimovData", "Pairing nui: " + s + ", with glob: " + s2 + ", from constraint: " + s3);
...
//save the snapshots of nominal parameters, but only if they're not already saved
    ...
    if (!w->loadSnapshot("nominalGlobs")){
        WriteInfoStatus("RunSig::makeAsimovData", "nominalGlobs doesn't exist. Saving snapshot.");
        w->saveSnapshot("nominalGlobs",*mc->GetGlobalObservables());
    }
    else w->loadSnapshot("tmpGlobs");
    if (!w->loadSnapshot("nominalNuis")){
        WriteInfoStatus("RunSig::makeAsimovData", "nominalNuis doesn't exist. Saving snapshot.");
...
//loop over the nui/glob list, grab the corresponding variable from the tmp ws, and set the glob to the value of the nui
    ...
    if (nrNuis != glob_list.getSize()){
        WriteErrorStatus("RunSig::makeAsimovData", "nui_list.getSize() != glob_list.getSize()!");
        return NULL;
    }
...
//save the snapshots of conditional parameters
    WriteInfoStatus("RunSig::makeAsimovData", "Saving conditional snapshots");
    WriteInfoStatus("RunSig::makeAsimovData", "Glob snapshot name = conditionalGlobs"+muStrProf.str());
    WriteInfoStatus("RunSig::makeAsimovData", "Nuis snapshot name = conditionalNuis"+muStrProf.str());
    ...
    WriteDebugStatus("RunSig::makeAsimovData", "Making asimov");
    ...
    if (!simPdf){
        ...
        for(int jj=0; jj<thisObs->numBins(); ++jj){
            ...
            if (thisNorm*expectedEvents <= 0){
                ...
                WriteWarningStatus("RunSig::makeAsimovData", "Detected bin with zero expected events (" + std::to_string(thisNorm*expectedEvents) + ") ! Please check your inputs. Obs = " + s + ", bin = " + std::to_string(jj));
            }
        ...
        if(asimovData->sumEntries()!=asimovData->sumEntries()){
            WriteErrorStatus("RunSig::makeAsimovData", "sum entries is nan");
            exit(1);
        }
