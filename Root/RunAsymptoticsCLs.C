        ...
        while (fabs(qmu95_guess-qmu95_pre) > 2*qmu95_guess*precision){
            ...
            for (map<double, double>::iterator itr=guess_to_corr.begin();itr!=guess_to_corr.end();itr++){
                if (fabs(itr->first - qmu95_guess) < 2*qmu95_guess*precision) {
                    damping_factor *= 0.8;
                    if (TRExFitter::DEBUGLEVEL > 2) WriteVerboseStatus("RunAsymptoticCLs::getQmu95", "Changing damping factor to " + std::to_string(damping_factor) + ", nrDamping = " + std::to_string(nrDamping));
                    if (nrDamping++ > 10){
                        nrDamping = 1;
                        damping_factor = 1.0;
                    }
...
double LimitsCLs::calcCLs(double qmu_tilde, double sigma, double mu){
    ...
    if (TRExFitter::DEBUGLEVEL > 2){ // need to explicitelly do this otherwise the code is super slow
...
double LimitsCLs::calcPmu(double qmu, double sigma, double mu){
    ...
    if (TRExFitter::DEBUGLEVEL > 2) WriteVerboseStatus("RunAsymptoticCLs::calcPmu", "for pmu, qmu = " + std::to_string(qmu) + ", sigma = " + std::to_string(sigma) + ", mu = " + std::to_string(mu) + ", pmu = " + std::to_string(pmu));
    return pmu;
}
