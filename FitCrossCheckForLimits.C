...
namespace LimitCrossCheck{  
  ...
  void PlotHistosAfterFitEachSubChannel(bool IsConditionnal, double mu){
	...
	fnuisPar << "$\\mu$ & $" << firstPOI->getVal() << "^{+" << firstPOI->getErrorHi() << "}_{-" << fabs(firstPOI->getErrorLo()) << "}$ \\\\" << endl;
        fnuisParAndCorr << firstPOI->GetName()<<"  " << firstPOI->getVal() << " +" << firstPOI->getErrorHi() << " -" << fabs(firstPOI->getErrorLo()) << "" << endl;
...
// 	if ( (varname.find("gamma_stat")!=string::npos) ) continue;
	...
	TString vname1=vname;
        vname1.ReplaceAll("_","\\_");
	fnuisPar.precision(3);
	fnuisPar << vname1 << " & $" << pull << "^{+" << fabs(errorHi) << "}_{-" << fabs(errorLo)  << "}$ \\\\" << endl;
        fnuisParAndCorr << vname << "  " << pull << " +" << fabs(errorHi) << " -" << fabs(errorLo)  << "" << endl;
